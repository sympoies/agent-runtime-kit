...
name: project-devlog
description: >
  Append a development-log entry to docs/source/devlog/<YYYY-MM>.md after a
  session produces a durable outcome for agent-runtime-kit. Skip trivial or
  transient work.
...
This skill enforces `docs/source/devlog/README.md`.

## Contract

Prereqs:

- Run inside the `agent-runtime-kit` git worktree with
  `docs/source/devlog/` present.
...
- A new `## YYYY-MM-DD - <title>` entry at the top of
  `docs/source/devlog/<YYYY-MM>.md`, creating and indexing the month when
  needed.
...
- The entry only repeats a commit or normative document without adding
  context, evidence, or useful links.
...
2. Keep normative docs current first. The devlog records the narrative and
   does not own the current runtime contract, policy, setup, or runbook.
3. Resolve `docs/source/devlog/$(date +%Y-%m).md`. When creating a month, add a
   `# Development log - YYYY-MM` header and add it to the month index in
   `docs/source/devlog/README.md`.
...
7. If committing separately, commit only the devlog file or new-month index
   alongside it, using the semantic-commit workflow.

## Boundary

This skill only appends under `docs/source/devlog/` and updates the month index
when a new month is created. It must not edit code, normative docs, runtime-kit
manifests, rendered product output, global runtime homes, credentials,
sessions, or cache state.
