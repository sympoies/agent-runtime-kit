...
description: >
  Audit and update the agent-runtime-kit version baseline: check README and
  harness-shape mirrors against runtime-roots.yaml, nils-cli-pin.yaml, and
  surfaces.yaml; report installed-vs-latest; and guide product-floor bumps.
...
The deterministic audit checks the hand-maintained harness coverage tables
against these per-surface values. `SUPPORT_MATRIX.md` and its golden remain
renderer-owned.
