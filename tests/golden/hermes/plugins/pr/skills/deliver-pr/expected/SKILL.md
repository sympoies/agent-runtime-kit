...
- Every provider-visible specialist body is the canonical
  ...
  second bullet renderer. That bundle call must bind `--reviewable`, `--lens`,
  `--lens-verdict`, `--scope`, and `--evidence-reviewed`; a missing flag renders
  a placeholder header into the published review, and `--evidence-reviewed` must
  be a portable identifier rather than an absolute local path.
