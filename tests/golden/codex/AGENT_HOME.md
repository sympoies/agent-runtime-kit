...
- For an active goal's required decision, use `request_user_input` when available, not a plain-text question. Otherwise follow the injected blocked-audit contract rather than stop prematurely; only the user's later explicit selection authorizes the shown action.
