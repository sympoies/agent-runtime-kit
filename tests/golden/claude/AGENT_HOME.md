...
- For an active goal's required decision, use `AskUserQuestion` when available; never end with a plain-text question then. If unavailable, follow the injected goal's blocked-audit contract rather than stop prematurely. Only the user's later explicit selection authorizes the shown action.
