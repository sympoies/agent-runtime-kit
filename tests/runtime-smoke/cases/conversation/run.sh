...
run_main_agent_mode_probe() {
  ...
  grep -Fq 'candidate conflict check cleared or was retained as bounded evidence' "$protocol"
  grep -Fq 'exact new session ID,' "$protocol"
  grep -Fq 'incarnation, working directory, and `enforce` mode' "$protocol"
  grep -Fq 'target capability or claims on the target' "$protocol"
  grep -Fq 'target worker then runs its own authenticated self-check' "$protocol"
  grep -Fq 'released or expired claim must be reacquired and verified' "$protocol"
  grep -Fq 'before another mutation turn' "$protocol"
  grep -Fq 'before that handoff is a failed ownership proof' "$protocol"
  grep -Fq 'session created, prompt delivery unverified' "$protocol"
  grep -Fq 'paste without Enter, verified by envelope or character count' "$protocol"
  grep -Fq 'success is transport evidence only' "$protocol"
  grep -Fq 'On mismatch, truncation, interference, missing readiness' "$protocol"
  grep -Fq 'permission, hook, or other startup' "$protocol"
  grep -Fq 'typed result instead of hand-running the sequence' "$protocol"
  grep -Fq 'never downgrades a not-ready typed result into a' "$protocol"
