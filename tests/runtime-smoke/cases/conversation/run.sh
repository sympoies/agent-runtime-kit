...
run_main_agent_mode_probe() {
  ...
  grep -Fq 'candidate conflict check before `start`' "$protocol"
  grep -Fq 'exact new session ID,' "$protocol"
  grep -Fq 'incarnation, working directory, and `enforce` mode' "$protocol"
  grep -Fq 'The launcher never uses the target capability or claims on the target' "$protocol"
  grep -Fq 'The target worker runs its authenticated self-check and acquires and verifies' "$protocol"
  grep -Fq 'released or expired claim must be reacquired and verified' "$protocol"
  grep -Fq 'before another mutation turn' "$protocol"
  grep -Fq 'Interference or deletion before that handoff is a failed ownership proof' "$protocol"
