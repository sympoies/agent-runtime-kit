...
Keep detailed evidence in typed child records and link them from the envelope. Route an important unresolved or reusable workflow gap to the earliest owning test, diagnostic, policy, skill, or ordinary issue when authorized; do not create a parallel inbox. This hook is a reminder only; do not auto-generate provider records. Follow the evidence control-plane policy and the released skill-usage CLI contract owned by the parent workflow."""
    sys.stdout.write(
        json.dumps(
            {
                "hookSpecificOutput": {
