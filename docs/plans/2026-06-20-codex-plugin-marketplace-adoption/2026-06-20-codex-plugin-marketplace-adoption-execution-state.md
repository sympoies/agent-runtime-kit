...
- 2026-07-22: trusted hook materialization retired the `hooks.shared-scripts` link-map entry so live handlers no longer execute through a source-checkout symlink. The clean Codex skill-surface doctor now reports 20/20 checks; `SHAPE_EXPECTED_MIN_CHECKS` is 20 because handler trust is validated by `agent-hook doctor` and the materialization acceptance tests instead.
