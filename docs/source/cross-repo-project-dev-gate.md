# Cross-Repository Project-Dev Gate

Canonical decision and delivery-handoff record for the cross-repository
project-dev gate. Promoted to `docs/source/` on 2026-09-14 because its own
Retention Intent declares it durable: content worth keeping takes the `canonise`
exit, which moves it out of `docs/discussions/`.

- First settled 2026-07-22, from Main Agent reproduction and a converged
  implementation contract.
- Outstanding at promotion: Main Agent review, integration, deployment
  acceptance, and the separately owned bounded nils-cli diagnostic enrichment.
...
- `docs/source/cross-repo-project-dev-gate.md`: this document, the
  implementation-readiness source, promoted here on 2026-09-14.
...
handoff record. It now lives in `docs/source/`, which is where that intent is
honoured — a capture cannot declare itself retained inside
`docs/discussions/`. Update or supersede it explicitly if the architecture
changes.
