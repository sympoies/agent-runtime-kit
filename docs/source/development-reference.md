...
On-demand setup and operations reference for `agent-runtime-kit`: host setup,
runtime synchronization, private overlays, render commands, test matrices,
coupled `nils-cli` work, and release detail. For routine contributor principles
and validation routing, start at the root
[`DEVELOPMENT.md`](../../DEVELOPMENT.md). Repository purpose, runtime model,
and directory map live in [`README.md`](../../README.md).
...
[`macos-agent-bootstrap-prompt.md`](macos-agent-bootstrap-prompt.md).
