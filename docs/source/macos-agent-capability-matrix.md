...
| SSH journal step accumulation | supported | From `macos-agent` 1.27.12 each SSH `exec` merges its transferred journal into `--out-dir` instead of replacing it, so a chained SSH flow reads its own stability rate back from `steps.jsonl`; the returned `journal_step` is the remapped local id. A transfer from a different execution tuple, or one that cannot fit the rotation bound whole, is refused rather than merged. | Live three-call chained SSH flow reaching `step-000003` under one `run_id` with `journal review` clean, plus the adapter merge unit tests. |
